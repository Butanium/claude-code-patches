#!/usr/bin/env python3
"""monitor-undefer: Monitor ships loaded, not deferred behind ToolSearch.

One trivial `claude -p` turn, then the dumped request body is read.
  patched: Monitor and its full input schema are in the request's `tools`
  stock:   Monitor is absent there (the model must ToolSearch for it first)
...
def run(binary: Path) -> Verdict:
    return undefer_verdict(binary, "Monitor")
