#!/usr/bin/env python3
"""taskstop-undefer: TaskStop ships loaded, not deferred behind ToolSearch.

One trivial `claude -p` turn, then the dumped request body is read.
  patched: TaskStop and its full input schema are in the request's `tools`
  stock:   TaskStop is absent there (the model must ToolSearch for it first)
...
def run(binary: Path) -> Verdict:
    return undefer_verdict(binary, "TaskStop")
